...
\begin{frame}
\frametitle{MIB Plana vs Modelo CIM}
\vfill
\begin{center}
\includegraphics[width=0.97\textwidth,height=0.70\textheight,keepaspectratio]{../Diagramas/a13-mib-vs-cim.png}
\end{center}
\vfill
\end{frame}
...
\begin{frame}
\frametitle{Os Três Paradigmas de Gestão}
\vfill
\begin{center}
\includegraphics[width=0.97\textwidth,height=0.70\textheight,keepaspectratio]{../Diagramas/a13-paradigmas.png}
\end{center}
\vfill
\end{frame}
...
\begin{itemize}
    ...
    \item Containers: namespaces isolam o que se \emph{vê}; cgroups limitam o que se \emph{consome} (Aula 04).
    \item Imagens em camadas (OverlayFS), arquitetura Docker (dockerd/containerd/runc) e o standard OCI.
    \item Containers de aplicação (Docker) vs de sistema (LXC): mesma tecnologia, usos distintos.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item \textbf{Abstração em camadas}: container sobre VM sobre hypervisor sobre hardware -- cada camada esconde a de baixo e cobra algum overhead.
...
\begin{itemize}
    ...
    \item Prestar atenção às ligações entre aulas destacadas ao longo do semestre (ex.\ SLA $\rightarrow$ disponibilidade, containers $\rightarrow$ Kubernetes $\rightarrow$ SDN).
