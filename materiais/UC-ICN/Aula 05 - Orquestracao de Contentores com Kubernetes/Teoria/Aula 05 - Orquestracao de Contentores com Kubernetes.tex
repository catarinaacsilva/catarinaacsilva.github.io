...
\title{Orquestração de Containers com Kubernetes}
...
\begin{itemize}
    ...
    \item Cargas de trabalho não contínuas: Job e CronJob.
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item O Docker, por si só, gere containers num único host — não decide onde correr cada container, nem reage a falhas.
...
\begin{itemize}
    ...
    \item \textbf{kubelet}: agente que garante que os containers descritos para o nó estão de facto a correr.
    \item \textbf{kube-proxy} e o \textbf{container runtime} (ex.: containerd) completam o nó, tratando de rede e execução de containers.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Arquitetura do Cluster}
\vfill
\begin{center}
\includegraphics[width=0.97\textwidth,height=0.70\textheight,keepaspectratio]{../Diagramas/a05-arquitetura.png}
\end{center}
\vfill
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item \textbf{Pod}: unidade mínima de execução; agrupa um ou mais containers que partilham rede e armazenamento.
...
\begin{frame}
\frametitle{Do Manifesto ao Pod em Execução}
\vfill
\begin{center}
\includegraphics[width=0.97\textwidth,height=0.70\textheight,keepaspectratio]{../Diagramas/a05-objetos.png}
\end{center}
\vfill
\end{frame}
...
\begin{itemize}
    ...
    \item \textbf{Liveness probe}: verifica se o container continua saudável; se falhar, o kubelet reinicia-o.
    \item \textbf{Readiness probe}: verifica se o container está pronto para receber tráfego, antes de ser incluído num Service.
...
\begin{frame}
\frametitle{O Ciclo, Visualmente}
\vfill
\begin{center}
\includegraphics[width=0.97\textwidth,height=0.70\textheight,keepaspectratio]{../Diagramas/a05-reconciliacao.png}
\end{center}
\vfill
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item \textbf{ConfigMap}: guarda dados de configuração não sensíveis, separando-os da imagem do container.
    ...
    \item Ambos podem ser expostos ao container como variáveis de ambiente ou como ficheiros montados.
...
\begin{frame}
\frametitle{Cargas de Trabalho Não Contínuas: Job e CronJob}
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item Nem toda a carga de trabalho é um serviço permanente: um \textbf{Job} corre um Pod até este terminar com sucesso, em vez de o manter continuamente ativo.
    \item O Kubernetes garante que o Job é concluído mesmo que um Pod falhe a meio -- recria-o e tenta de novo, respeitando um número máximo de tentativas.
    \item Útil para tarefas pontuais: migrações de base de dados, processamento em lote, geração de relatórios.
    \item \textbf{CronJob}: agenda Jobs para correr periodicamente, com uma sintaxe semelhante ao \texttt{cron} do Unix (ex.: backups noturnos).
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item \textbf{Limits}: teto máximo que o container pode consumir, aplicado através dos cgroups vistos na Aula 04.
    \item Exceder o limite de memória leva à terminação do container (OOM kill); exceder o de CPU resulta apenas em \textit{throttling}.
...
\begin{itemize}
    ...
    \item \textbf{HashiCorp Nomad}: orquestrador genérico, capaz de agendar não só containers mas também outras cargas de trabalho (binários, VMs).
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item Orquestração é necessária para gerir containers de forma fiável em múltiplos hosts.
...
\begin{itemize}
    ...
    \item ConfigMaps, Secrets e PersistentVolumes separam configuração e dados do ciclo de vida dos Pods; Jobs e CronJobs cobrem cargas de trabalho pontuais ou periódicas.
