...
\begin{itemize}[itemsep=0.2cm]
  \item O cluster Kubernetes de dois nós (Guião 05), com \texttt{kubectl} configurado e o \emph{taint} do control plane removido (necessário para o exercício de anti-affinity).
...
\end{lstlisting}
  Repare que as credenciais \textbf{não} aparecem em texto simples neste manifesto: são lidas do Secret com \texttt{secretKeyRef}. Isto permite versionar o \texttt{worker.yaml} num repositório sem expor a password -- ao contrário do que aconteceria se a escrevesse em \texttt{value:}.
...
\end{lstlisting}
  ...
  \item \textbf{Forçar a distribuição por nós diferentes} com anti-affinity.
  Acrescente o bloco seguinte ao \texttt{spec.template.spec} do \texttt{worker.yaml}:
\begin{lstlisting}
      affinity:
        podAntiAffinity:
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Submeta os seguintes ficheiros:
\begin{lstlisting}
rabbitmq.yaml       worker.yaml
send_task.py        worker.py
publish_event.py    subscribe_event.py
\end{lstlisting}
  Junte uma captura da interface de gestão a mostrar mensagens acumuladas na fila.
