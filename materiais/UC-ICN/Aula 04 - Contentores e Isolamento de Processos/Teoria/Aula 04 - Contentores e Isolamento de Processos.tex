...
\title{Containers e Isolamento de Processos}
...
\begin{itemize}
    ...
    \item Containers vs máquinas virtuais.
    \item Primitivas Linux de isolamento: namespaces e cgroups.
    \item Union filesystems, imagens em camadas e as suas limitações.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Boas práticas e segurança de containers.
    \item Containers de aplicação vs containers de sistema.
    \item OCI e a convergência com máquinas virtuais leves.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item Containers partilham o kernel do sistema operativo do host, ao contrário das VMs, que virtualizam hardware e correm um kernel próprio.
...
\begin{itemize}
    ...
    \item \textbf{USER}: mapeamento de identificadores de utilizador e grupo, permitindo root dentro do container sem privilégios no host.
    \item Um container Docker é, na prática, um processo Linux normal ao qual foram atribuídos vários destes namespaces.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Sem cgroups, um único container descontrolado poderia consumir toda a memória ou CPU do host.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Namespaces Isolam, Cgroups Limitam}
\vfill
\begin{center}
\includegraphics[width=0.97\textwidth,height=0.70\textheight,keepaspectratio]{../Diagramas/a04-namespaces-cgroups.png}
\end{center}
\vfill
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item Todos os containers de um host partilham o mesmo kernel: uma vulnerabilidade no kernel é uma superfície de ataque comum.
    \item \textbf{Capabilities}: o Linux divide os privilégios de root em capacidades individuais (ex.\ \texttt{CAP\_NET\_ADMIN}); um container deve receber apenas as estritamente necessárias.
    ...
    \item Quando é exigido isolamento forte (ex.\ entre clientes diferentes), uma VM continua a ser mais segura do que um container.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Ao correr um container, é adicionada uma camada de escrita (writable layer) no topo, específica dessa instância.
...
\begin{frame}
\frametitle{OverlayFS na Prática: Limitações}
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item No host, cada container corresponde a um diretório em \texttt{/var/lib/docker/overlay2/<ID>/}: \texttt{merged} (a vista final, dentro do container) e \texttt{diff} (só as alterações dessa camada).
    \item OverlayFS não suporta todas as operações POSIX -- é uma união de diretórios, não um sistema de ficheiros completo.
    \item Abrir um ficheiro para escrita depois de já estar aberto para leitura obriga a copiá-lo primeiro para a camada de escrita: os dois descritores deixam de apontar para o mesmo ficheiro.
    \item \texttt{rename} não é suportado diretamente: a aplicação tem de copiar e depois apagar o original, o que é mais lento e ocupa mais espaço.
    \item Consequência prática: aplicações com padrões de I/O intensivos beneficiam de um volume (fora do overlay) em vez da camada de escrita.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item \textbf{containerd}: runtime de alto nível, responsável pelo ciclo de vida dos containers (criação, execução, paragem).
    \item \textbf{runc}: runtime de baixo nível que efetivamente cria o container, aplicando namespaces e cgroups.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item A imagem resultante pode depois ser instanciada em um ou mais containers com \texttt{docker run}.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item \textbf{bridge}: rede virtual isolada no host, usada por omissão; os containers comunicam entre si e saem para o exterior por NAT.
    \item Numa rede bridge \emph{personalizada}, o Docker fornece resolução de nomes: um container alcança outro pelo seu nome.
    \item \textbf{host}: o container usa diretamente a pilha de rede do host, sem isolamento de rede (sem namespace NET próprio).
    \item \textbf{none}: sem qualquer interface de rede, para cargas de trabalho totalmente isoladas.
    \item Publicação de portas (\texttt{-p 8080:80}) mapeia uma porta do host para uma porta do container.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item A camada de escrita de um container desaparece quando este é removido -- por omissão, os dados não persistem.
    \item \textbf{Volumes}: geridos pelo Docker, guardados numa área própria do host; a forma recomendada de persistir dados.
    \item \textbf{Bind mounts}: mapeiam um diretório concreto do host para dentro do container; úteis em desenvolvimento, mas dependentes da estrutura do host.
    \item \textbf{tmpfs}: armazenamento apenas em memória, que nunca chega ao disco -- adequado a dados sensíveis ou temporários.
    \item Separar dados do ciclo de vida do container é essencial para tratar containers como recursos descartáveis.
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Containers de Aplicação vs de Sistema}
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item \textbf{Container de aplicação} (Docker): corre tipicamente um único processo/serviço; é efémero e descartável.
    \item Não tem um sistema de init completo; o ciclo de vida do container é o ciclo de vida desse processo.
    \item \textbf{Container de sistema} (LXC, visto na Aula 03): comporta-se como uma máquina Linux completa, com \texttt{systemd} e múltiplos serviços.
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item Iniciativa que define especificações abertas e neutras de fornecedor para formatos de containers.
    \item \textbf{image-spec}: define o formato de uma imagem de container, independente da ferramenta que a construiu.
    \item \textbf{runtime-spec}: define como um runtime deve criar e executar um container a partir dessa imagem.
...
\begin{frame}
\frametitle{Containers como Máquinas Virtuais Leves}
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item A fronteira entre container e VM (Aula 03) está a esbater-se: se o runtime e o formato de imagem seguem OCI, a implementação por baixo deixa de ser relevante para quem usa o container.
    \item \textbf{Kata Containers}: implementa a mesma interface OCI, mas cada "container" corre dentro de uma micro-VM leve (com o seu próprio kernel), combinando a portabilidade de Docker com o isolamento de hardware de uma VM.
    \item \textbf{gVisor}: outra abordagem híbrida -- intercepta as \textit{syscalls} do container num kernel de utilizador, sem chegar a expor o kernel real do host.
    \item Motivação: cargas multi-tenant (ex.\ funções de clientes diferentes no mesmo host) em que o isolamento fraco de um container tradicional é insuficiente.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item Containers partilham o kernel do host, oferecendo menor overhead e arranque quase instantâneo face a VMs.
    \item Namespaces isolam o que um processo vê (PID, NET, MNT, UTS, IPC, USER); cgroups limitam o que consome (CPU, memória, I/O).
    \item OverlayFS e imagens em camadas permitem reutilização eficiente de camadas comuns entre imagens, à custa de limitações POSIX (escrita concorrente, rename) que justificam o uso de volumes para I/O intensivo.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item Docker articula dockerd, containerd e runc para transformar um Dockerfile numa imagem e, depois, num container em execução.
    ...
    \item O isolamento de containers não é absoluto: capabilities, seccomp e AppArmor/SELinux reforçam-no, mas uma VM continua a isolar melhor -- por isso soluções como Kata Containers e gVisor aproximam as duas abordagens.
    \item OCI garante portabilidade das imagens entre diferentes runtimes de containers.
\end{itemize}
\end{frame}
