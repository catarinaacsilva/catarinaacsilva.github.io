...
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item No host, cada contentor corresponde a um diretório em \texttt{/var/lib/docker/overlay2/<ID>/}: \texttt{merged} (a vista final, dentro do contentor) e \texttt{diff} (só as alterações dessa camada).
...
\begin{frame}
\frametitle{Contentores como Máquinas Virtuais Leves}
\begin{itemize}
    \setlength\itemsep{0.4cm}
    \item A fronteira entre contentor e VM (Aula 03) está a esbater-se: se o runtime e o formato de imagem seguem OCI, a implementação por baixo deixa de ser relevante para quem usa o contentor.
    \item \textbf{Kata Containers}: implementa a mesma interface OCI, mas cada "contentor" corre dentro de uma micro-VM leve (com o seu próprio kernel), combinando a portabilidade de Docker com o isolamento de hardware de uma VM.
    \item \textbf{gVisor}: outra abordagem híbrida -- intercepta as \textit{syscalls} do contentor num kernel de utilizador, sem chegar a expor o kernel real do host.
    \item Motivação: cargas multi-tenant (ex.\ funções de clientes diferentes no mesmo host) em que o isolamento fraco de um contentor tradicional é insuficiente.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item O isolamento de contentores não é absoluto: capabilities, seccomp e AppArmor/SELinux reforçam-no, mas uma VM continua a isolar melhor -- por isso soluções como Kata Containers e gVisor aproximam as duas abordagens.
