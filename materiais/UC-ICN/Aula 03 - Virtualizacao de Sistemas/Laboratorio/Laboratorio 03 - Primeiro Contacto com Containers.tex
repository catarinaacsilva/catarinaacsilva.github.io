...
\newtcolorbox{registo}{
  ...
  boxrule=0.6pt, arc=2pt, left=7pt, right=7pt, top=6pt, bottom=6pt,
  title={\small\bfseries Pare e observe}, coltitle=TextEspresso,
...
\begin{document}
\labtitle{03}{Primeiro Contacto com Containers}{Aula 03}
...
O trabalho decorre no computador pessoal de cada aluno e não requer acesso ao servidor da unidade curricular. Destina-se a exploração durante a aula, não constituindo trabalho a entregar. Não se pressupõe experiência prévia com containers.

\begin{nota}
O laboratório utiliza Docker por ser a implementação de containers de instalação mais simples em qualquer sistema operativo. Os mecanismos de núcleo que a sustentam, designadamente \emph{namespaces} e \emph{cgroups}, constituem matéria da Aula 04. O objetivo neste momento não é dominar a ferramenta, mas observar o comportamento que a teoria descreve.
\end{nota}
...
\begin{itemize}
  \item Executar e inspecionar um container, distinguindo imagem de instância em execução.
  \item Demonstrar experimentalmente a partilha do núcleo entre container e anfitrião.
  \item Identificar, na sua própria máquina, onde se situa o hypervisor da Aula 03.
  \item Publicar um serviço em rede a partir de um container e justificar o mapeamento de portos.
  \item Construir uma imagem a partir de um \texttt{Dockerfile} e explicar a sua estrutura em camadas.
  \item Aplicar limites de recursos a um container e observar o seu efeito.
\end{itemize}

\section*{Como utilizar este guião}

O guião não constitui trabalho a entregar. Destina-se a ser percorrido durante a aula, ao ritmo de cada aluno, com acompanhamento da docente.

Ao longo do texto surgem caixas \emph{Pare e observe}. Assinalam os momentos em que interessa deter a execução e interpretar o resultado obtido, isoladamente ou em discussão com os colegas, antes de prosseguir. São esses momentos, e não a execução dos comandos, que constituem o objeto do trabalho.

\section*{Percurso}

A figura da página seguinte apresenta a sequência do guião. Os passos progridem da simples execução de um comando até ao controlo dos recursos atribuídos, e o passo~3 assinala a observação central, aquela que revela a arquitetura de virtualização do próprio computador.

\begin{landscape}
\thispagestyle{empty}
\vspace*{\fill}
\begin{center}
\includegraphics[width=\linewidth]{../Diagramas/a03-percurso-laboratorio.png}
\end{center}
\vspace*{\fill}
\end{landscape}
...
A primeira linha merece atenção. A imagem não existia localmente, pelo que foi transferida de um repositório remoto antes da execução. Esta separação entre \textbf{imagem}, o conjunto de ficheiros imutável que serve de modelo, e \textbf{container}, a instância em execução criada a partir dela, é análoga à distinção entre um ficheiro de disco virtual e uma máquina virtual em execução, discutida na Aula 03.

Confirme que o container terminou mas continua registado:
...
O primeiro comando lista apenas containers em execução e não apresentará resultados. O segundo inclui os já terminados, entre os quais o \texttt{hello-world} com estado \texttt{Exited}.

\begin{nota}
Um container termina quando o processo que lhe deu origem termina. Não existe, ao contrário de uma máquina virtual, um sistema de arranque a manter a instância ativa na ausência de trabalho.
\end{nota}

\section{Observar o isolamento}

Inicie um container interativo baseado em Alpine Linux, uma distribuição de dimensão reduzida:

\begin{terminal}
docker run -it --rm alpine sh
\end{terminal}

A opção \texttt{-it} liga o terminal ao container e \texttt{--rm} remove-o automaticamente ao terminar. A linha de comandos passa a ser a do interior do container.

Execute, já dentro do container:
...
\begin{registo}
\textbf{Observação 1.} Observe o resultado de \texttt{ps aux} dentro do container. Quantos processos são visíveis? Compare com o número de processos em execução no seu computador. Justifique a diferença em termos do isolamento descrito na Aula 03.
\end{registo}

\textbf{Não termine ainda este container.} A secção seguinte utiliza-o.
...
Registe a versão apresentada. Termine o container com \texttt{exit} e execute, agora no seu computador:
...
\textbf{Em Linux.} As duas versões coincidem exatamente. O container não possui núcleo próprio: executa sobre o núcleo do anfitrião, que apenas lhe apresenta uma vista isolada dos recursos. Alpine e a distribuição instalada partilham o mesmo núcleo e diferem somente no espaço de utilizador.

\textbf{Em Windows ou macOS.} As duas versões diferem, e o núcleo comunicado dentro do container é um núcleo Linux, embora o sistema operativo instalado não o seja. A explicação encontra-se na aula: o Docker Desktop executa uma máquina virtual Linux sobre um hypervisor do sistema, o Hyper-V ou WSL~2 em Windows, o Virtualization.framework em macOS. O container partilha o núcleo dessa máquina virtual, e não o do computador.

\begin{nota}
Daqui decorre uma observação de alcance mais geral. Em Windows e macOS coexistem, na mesma máquina, os dois modelos de virtualização estudados na aula: virtualização completa, que sustenta a máquina virtual Linux, e virtualização ao nível do sistema operativo, que produz o container no seu interior. A afirmação corrente de que containers dispensam máquinas virtuais é, nestes sistemas, incorreta.
\end{nota}

\begin{registo}
\textbf{Observação 2.} Compare as duas versões de núcleo obtidas, tendo presente o seu sistema operativo. Diga se coincidem e justifique. Caso não coincidam, indique onde se situa o hypervisor e que papel desempenha.
\end{registo}
...
\begin{registo}
\textbf{Observação 3.} Repare na dimensão de \texttt{nginx:alpine} e o número de camadas apresentado por \texttt{docker history}. Explique por que motivo duas imagens derivadas da mesma base ocupam, em conjunto, menos espaço do que a soma das suas dimensões individuais.
\end{registo}

\section{Publicar um serviço}

Inicie o servidor web em segundo plano, mapeando o porto 8080 do computador para o porto 80 do container:
...
O mapeamento é necessário porque o container possui uma pilha de rede própria, com endereço distinto do da máquina. Sem \texttt{-p}, o serviço permanece acessível apenas no interior da rede de containers. A opção estabelece uma regra de reencaminhamento entre o porto do anfitrião e o porto do container.

Consulte os registos e o interior do container em execução:
...
Dentro do container, confirme que o servidor está ativo e observe os ficheiros servidos:
...
<body>
  ...
  <p>Pagina servida a partir de um container construido localmente.</p>
</body>
</html>
\end{terminal}
...
\begin{registo}
\textbf{Observação 4.} Confirme que a página servida é a sua. Execute \texttt{docker images} e compare a dimensão de \texttt{icn-site:v1} com a de \texttt{nginx:alpine}. Explique a diferença observada com base no modelo de camadas.
\end{registo}
...
A camada de escrita de um container é descartada com ele. Confirme experimentalmente:
...
O último comando falha: o ficheiro não existe na nova instância, criada a partir da imagem original e sem qualquer memória do container anterior.

Repita com um volume, que persiste independentemente do ciclo de vida do container:
...
O conteúdo sobrevive agora à destruição e recriação do container.

\begin{registo}
\textbf{Observação 5.} Note o que sucedeu em cada um dos dois casos. Relacione o resultado com a distinção, estabelecida na Aula 03, entre o disco de uma máquina virtual e o estado volátil de uma instância.
\end{registo}

\section{Limitar recursos}

Execute um container com memória e processador limitados:
...
A coluna \texttt{MEM USAGE / LIMIT} apresenta o limite imposto e não a memória total do computador. O container não consegue exceder o valor declarado: caso o tente, o seu processo é terminado pelo sistema.

Este mecanismo constitui a contrapartida, no modelo de containers, da atribuição de memória a uma máquina virtual. A diferença é relevante: uma máquina virtual reserva a memória declarada, ao passo que um container apenas fica impedido de a exceder, consumindo em cada instante o que efetivamente necessita.

\begin{registo}
\textbf{Observação 6.} Leia a saída de \texttt{docker stats --no-stream}. Compare o modelo de limite aqui observado com o modelo de reserva de uma máquina virtual, e indique qual dos dois permite maior densidade de instâncias no mesmo anfitrião.
\end{registo}
...
\begin{atencao}
O comando \texttt{docker system prune -a} remove todas as imagens não associadas a containers em execução, incluindo imagens transferidas noutros contextos. Utilize-o apenas se não tiver trabalho por concluir noutros projetos.
\end{atencao}
...
Confirme com \texttt{docker ps} que o container se encontra em execução. Caso não apareça na lista, consulte \texttt{docker ps -a} e, em seguida, \texttt{docker logs <nome>} para identificar a causa da terminação.
...
\begin{itemize}
  ...
  \item[$\square$] \texttt{hello-world} executado, distinção entre imagem e container compreendida.
  \item[$\square$] Isolamento observado: hostname, processos e sistema de ficheiros próprios (Observação~1).
  \item[$\square$] Versões de núcleo comparadas dentro e fora do container, resultado interpretado (Observação~2).
...
A Aula 04 estabelece os mecanismos de núcleo que sustentam o comportamento aqui observado: os \emph{namespaces}, responsáveis pelo isolamento constatado na Observação~1, e os \emph{cgroups}, responsáveis pelos limites da Observação~6. As observações feitas neste laboratório servirão de referência concreta para essa exposição.
