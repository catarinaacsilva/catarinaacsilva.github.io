...
A fundamentação consta do slide anterior. Importa reter que a opção D, a execução de containers com utilizador não privilegiado, constitui medida de mitigação pertinente e recomendável, mas que não altera a natureza da fronteira: o núcleo permanece partilhado. A prática corrente na indústria, referida na resolução, consiste em combinar ambos os modelos, atribuindo uma máquina virtual por cliente e executando containers no seu interior.
}

\pagina{slides/slide-45.png}{
O Proxmox VE integra numa interface única os dois modelos analisados, o KVM para máquinas virtuais e o LXC para containers de sistema, o que o torna adequado a fins pedagógicos por permitir a comparação direta entre ambos. A gestão faz-se por interface web ou por linha de comandos, através de qm para instâncias KVM e pct para containers LXC. A plataforma suporta instantâneos, clones, modelos e migração entre nós, concretizando os mecanismos analisados ao longo desta aula, e constitui o ambiente que sustenta o trabalho de projeto da segunda metade do semestre.
}

\pagina{slides/slide-46.png}{
O laboratório associado a esta aula decorre em equipamento próprio de cada aluno e não requer acesso ao servidor da unidade curricular. O seu objetivo não é o domínio da ferramenta, mas a observação direta do comportamento que a exposição descreveu. Recorre a Docker por ser a implementação de containers de instalação mais simples em qualquer sistema operativo, ficando os mecanismos de núcleo subjacentes, namespaces e cgroups, para a Aula 04. Não constitui trabalho a entregar: destina-se a ser percorrido durante a aula, ao ritmo de cada aluno. As caixas \emph{Pare e observe} distribuídas pelo guião assinalam os momentos em que interessa deter a execução e interpretar o resultado, isoladamente ou em discussão, antes de prosseguir.
}

\pagina{slides/slide-47.png}{
O diagrama apresenta a sequência completa do guião numa única imagem, com os passos dispostos em progressão ascendente. A leitura da esquerda para a direita acompanha o aumento de complexidade e, simultaneamente, uma mudança de natureza do trabalho: os primeiros passos limitam-se a observar o que o sistema faz, ao passo que os últimos passam a controlá-lo, construindo imagens próprias e impondo limites de recursos. O passo~3 encontra-se assinalado por constituir a observação central, aquela que revela se existe, ou não, uma máquina virtual interposta entre o container e o computador. As duas caixas do canto superior esquerdo estabelecem a correspondência com a teoria: os três primeiros passos ilustram matéria desta aula, os restantes antecipam a Aula 04.
}

\pagina{slides/slide-48.png}{
A enumeração detalha os sete passos representados no diagrama anterior. Começa pela distinção entre imagem e instância em execução, análoga à distinção entre ficheiro de disco virtual e máquina virtual ativa. Prossegue com a observação do isolamento de processos, sistema de ficheiros e identificação de máquina. O terceiro ponto constitui a observação central do trabalho, a comparação da versão do núcleo dentro e fora do container, cujo resultado difere consoante o sistema operativo do computador e é objeto da questão 7. Seguem-se a estrutura em camadas de uma imagem, a publicação de um serviço em rede, a construção de imagem própria, a persistência de dados e a imposição de limites de recursos, estes últimos antecipando os cgroups da Aula 04.
}
...
A fundamentação consta do slide anterior. O resultado merece ser retido pelo que revela: em Windows e macOS coexistem, no mesmo equipamento, os dois modelos de virtualização estudados nesta aula, a virtualização completa que sustenta a máquina virtual Linux e a virtualização ao nível do sistema operativo que produz o container no seu interior. A afirmação corrente de que containers dispensam máquinas virtuais é, nestes sistemas, incorreta. Em Linux, a mesma verificação devolveria o núcleo do próprio anfitrião, confirmando a partilha direta.
}
...
A segunda metade retoma os resultados relativos ao I/O, ao armazenamento e à operação. O I/O admite quatro tratamentos, com compromissos entre desempenho e flexibilidade. O sobrecompromisso é admissível em memória, recurso reclamável, e exige vigilância acrescida em armazenamento, onde o espaço escrito não é recuperável. A migração por pré-cópia converge apenas quando a largura de banda excede a taxa de modificação de páginas. Por último, máquinas virtuais e containers diferem na natureza da fronteira de isolamento, do hardware ou do núcleo, o que determina a sua aplicação em cenários multi-cliente e justifica a sua combinação frequente.
}

\pagina{slides/slide-53.png}{
Conclui-se a exposição da Aula 03. O laboratório associado deve ser percorrido durante a aula, antes da Aula 04, que estabelece os mecanismos de núcleo responsáveis pelo comportamento nele observado: os namespaces, que produzem o isolamento constatado, e os cgroups, que impõem os limites de recursos aplicados. As observações feitas servirão de referência concreta para essa exposição.
}
